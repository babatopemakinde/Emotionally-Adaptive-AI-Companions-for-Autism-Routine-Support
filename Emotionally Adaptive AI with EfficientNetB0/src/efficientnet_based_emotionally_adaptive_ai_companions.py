# -*- coding: utf-8 -*-
"""EfficientNet-based Emotionally-Adaptive-AI-Companions.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gjJw5edB3mpIgl7MfwgsLy6Gu_bofW9Y
"""
...
def setup_environment():
    ...
    packages = [
        "gradio", "librosa", "tensorflow", "efficientnet", "shap",
...
def load_and_preprocess_data(zip_path, base_dir):
    ...
    if os.path.exists(zip_path):
        ...
        print(f"Dataset zip not found at {zip_path}. Generating synthetic samples for structure verification.")
...
class MultimodalEmotionSystem:
    def __init__(self, num_classes=6, class_indices=None, base_dir=None):
        ...
        self.img_model = self._build_efficientnet()
    ...
    def extract_vocal_features(self, audio_path):
        """Extracts MFCCs from audio, supports various formats via librosa/ffmpeg."""
        try:
    ...
    def generate_grad_cam(self, img_array, res=224):
        """Explainability: Generates and saves Grad-CAM visualizations."""
        try:
...
def launch_app(system, engine, base_dir):
    def process_multimodal(image, audio, routine, time_of_day, location, feedback):
        try:
            ...
            # 1. Facial Analysis (70% weight)
            img_input = cv2.resize(image, (224, 224))
            img_prep = tf.keras.applications.efficientnet.preprocess_input(np.expand_dims(img_input, 0))
    ...
    with gr.Blocks(theme=gr.themes.Soft(), title="Multimodal AI Companion") as demo:
        gr.Markdown("# 🧩 Multimodal Emotion AI (Autism Support)\n*Repository structure and full ML pipeline generated.*")
